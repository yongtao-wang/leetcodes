...
class Strings(object):
    ...
    def isMatchWildcard(self, s, p):
        ...
        sl, pl = len(s), len(p)
        while i < sl:
            if i < sl and j < pl and (s[i] == p[j] or p[j] == '?'):
                i += 1
                j += 1
            elif j < pl and p[j] == '*':
                s_star = i
    ...
    def licenseKeyFormatting(self, S, K):
        ...
        which we would like to format. The string S is composed of alphanumerical characters and dashes.
        ...
        To satisfy this requirement, we will reinsert dashes. Additionally, all the lower case letters
        ...
        if K == 0:
            return
        chars = [i.upper() for i in S if i.isalnum()]
        i, n = divmod(len(chars), K)
        res = [''.join(chars[:n])] if n != 0 else []
        index = n
        for _ in xrange(i):
            res.append(''.join(chars[index: index + K]))
            index += K
        return '-'.join(res)
